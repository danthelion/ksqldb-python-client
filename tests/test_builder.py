...
class TestSQLBuilder(unittest.TestCase):
    create_table_with_key = (
        ...
        "(registertime bigint, gender varchar, regionid varchar, userid varchar) WITH "
        "(kafka_topic='users', value_format='JSON', key='userid');"
    )
    create_table_without_key = (
        "CREATE TABLE users_original "
        "(registertime bigint, gender varchar, regionid varchar, userid varchar) "
        "WITH (kafka_topic='users', value_format='JSON');"
    )
    create_table_without_key_avro = (
        "CREATE TABLE users_original "
        "(registertime bigint, gender varchar, regionid varchar, userid varchar) "
        "WITH (kafka_topic='users', value_format='AVRO');"
    )
    create_table_without_key_delimited = (
        "CREATE TABLE users_original "
        "(registertime bigint, gender varchar, regionid varchar, userid varchar) "
        "WITH (kafka_topic='users', value_format='DELIMITED');"
    )
    create_stream_without_key = (
        "CREATE STREAM users_original "
        "(registertime bigint, gender varchar, regionid varchar, userid varchar) "
        "WITH (kafka_topic='users', value_format='JSON');"
    )
    create_stream_as_with_condition = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "timestamp='logtime') AS "
        "SELECT rowtime as logtime, * FROM pageviews_original "
        "WHERE userid like 'User_%' AND pageid like 'Page_%'"
    )
    create_stream_as_with_condition_select_star = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "timestamp='logtime') AS SELECT * FROM pageviews_original "
        "WHERE userid like 'User_%' AND pageid like 'Page_%'"
    )
    create_stream_as_without_condition = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "timestamp='logtime') AS SELECT rowtime as logtime, * FROM pageviews_original"
    )
    create_stream_as_without_condition_select_star = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "timestamp='logtime') AS SELECT * FROM pageviews_original"
    )
    create_stream_as_with_condition_with_partitions = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "partitions=5, timestamp='logtime') AS "
        "SELECT rowtime as logtime, * FROM pageviews_original "
        "WHERE userid like 'User_%' AND pageid like 'Page_%'"
    )
    create_stream_as_without_condition_partition_by = (
        "CREATE STREAM pageviews_valid WITH "
        "(kafka_topic='pageviews_valid', value_format='DELIMITED', "
        "timestamp='logtime') AS "
        "SELECT rowtime as logtime, * "
        "FROM pageviews_original PARTITION BY logtime"
    )
    create_stream_as_without_condition_avro = (
        "CREATE STREAM pageviews_valid "
        "WITH (kafka_topic='pageviews_valid', value_format='AVRO', "
        "timestamp='logtime')"
        " AS SELECT rowtime as logtime, * FROM pageviews_original"
    )
